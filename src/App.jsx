...
import HiddenServerGame from './HiddenServerGame';

// --- Terminal / Root Console Component ---
const ConsoleModal = ({ isOpen, onClose }) => {
  const [input, setInput] = useState('');
  const [history, setHistory] = useState([
    { text: "SACHIN_OS [v4.5.0] - SECURITY_AUDIT_PASSED", type: "sys" },
    { text: "Type 'help' for available commands.", type: "sys" }
  ]);
  ...
  const handleCmd = (e) => {
    if (e.key === 'Enter') {
      ...
      switch(cmd) {
        ...
        case 'ls': res = "experience/  arsenal/  iso_cert/  server_logs.log"; break;
        case 'whoami': res = SYSDATA.summary; break;
        case 'contact': res = `Primary_Endpoint: ${SYSDATA.email}`; break;
        case 'exit': onClose(); return;
        case 'clear': setHistory([]); setInput(''); return;
        default: res = `Access Denied: Command '${cmd}' not found in /bin/sh`;
      }
  ...
  return (
    ...
    >
      <motion.div 
        ...
        animate={{ scale: 1, y: 0 }} 
        className="w-full max-w-2xl bg-[#020617] border border-slate-800 rounded-2xl overflow-hidden font-mono text-[11px] shadow-2xl"
      >
        <div className="bg-slate-900 px-5 py-3 flex justify-between items-center border-b border-slate-800">
          ...
          <div className="flex gap-6 items-center">
            <a href={SYSDATA.linkedin} target="_blank" rel="noreferrer" className="text-sky-500 hover:text-white text-[9px] uppercase font-bold">LinkedIn_Connect</a>
            <button onClick={onClose} className="text-slate-500 hover:text-red-400 font-bold uppercase text-[9px]"> [ DISCONNECT ] </button>
          </div>
        </div>
...
export default function App() {
  ...
  return (
    <div className={`relative min-h-screen transition-colors duration-700 ${isDark ? 'bg-[#0f172a] text-slate-300' : 'bg-[#f8fafc] text-slate-600'}`}>
      ...
      {/* LAYER 0: Sandspiel Wind Background (Canvas) */}
      <WindBackground isDark={isDark} />

      {/* LAYER 1: Blueprint Grid */}
      <div 
        className="fixed inset-0 opacity-[0.03] z-[1] pointer-events-none" 
        style={{ 
      ...
      {/* LAYER 100+: HUD, Sidebar, and Mini-Game */}
      <MediaCheck isDark={isDark} />
      <GuidanceSystem />
      <HiddenServerGame isDark={isDark} />
      ...
      <nav className="fixed top-0 w-full z-50 px-6 md:px-12 py-8 flex justify-between items-center backdrop-blur-md border-b border-black/5">
        <h2 className={`font-mono text-[10px] tracking-[0.4em] font-bold uppercase ${isDark ? 'text-white' : 'text-slate-900'}`}>
          {SYSDATA.name} // INFRA_EXECUTIVE
        </h2>
        <div className="flex items-center gap-8">
          <button 
            onClick={() => setConsoleOpen(true)} 
            className="font-mono text-[10px] text-sky-500 hover:text-sky-400 border-b border-sky-500/30 uppercase font-bold tracking-widest"
          >
            Root_Access
          </button>
      ...
      <main className="max-w-4xl mx-auto px-6 pt-48 pb-32 relative z-10 text-left">
        ...
        {/* Career Tenure Section */}
        <section className="space-y-40">
          <h2 className="font-mono text-[10px] tracking-[0.6em] uppercase text-sky-500 font-bold mb-12">// Career_Deployment_Logs</h2>
              ...
              <ul className="text-[15px] space-y-6 opacity-80">
                {job.logs.map((log, j) => (
                  <li key={j} className="flex gap-4 items-start leading-relaxed text-left">
                    <span className="text-sky-500 font-mono text-[10px] mt-1">LOG_{j+1}</span>
                    <span>{log}</span>
                  </li>
                ))}
              </ul>
      ...
      <footer className="py-20 border-t border-white/5 relative z-10 opacity-40 font-mono text-[9px] uppercase tracking-[0.6em]">
        <div className="max-w-4xl mx-auto px-6 flex justify-between items-center">
          ...
          <span>Compliance_Status: ISO_27001_CERTIFIED</span>
        </div>
      </footer>
    </div>
  );
