...
const ConsoleModal = ({ isOpen, onClose }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
      <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} className="w-full max-w-2xl bg-[#020617] border border-slate-800 rounded-2xl shadow-2xl overflow-hidden font-mono text-[11px]">
        {/* Terminal Header */}
        ...
        <div className="h-80 p-8 overflow-y-auto custom-scrollbar text-left bg-black/40">
          ...
          <div className="flex gap-2 text-white"><span>$</span><input className="bg-transparent outline-none flex-1" value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleCmd} autoFocus spellCheck="false" /></div>
          <div ref={scrollRef} />
        </div>
      </motion.div>
    </motion.div>
    ...
    <div className={`min-h-screen transition-colors duration-700 ${isDark ? 'bg-[#0f172a] text-slate-300' : 'bg-[#f8fafc] text-slate-600'}`}>
      ...
      <main className="max-w-4xl mx-auto px-6 pt-48 pb-32 relative z-10 text-left">
        <section className="mb-48">
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-sky-500 font-mono text-[10px] uppercase tracking-[0.4em] mb-6 font-bold underline underline-offset-8 decoration-sky-500/20 text-left">Infrastructure_Manifesto</motion.p>
          ...
          {SYSDATA.experience.map((job, i) => (
            <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="border-l-2 border-sky-500/20 pl-12 relative group hover:border-sky-500 transition-colors duration-500">
              ...
              <ul className="text-[15px] space-y-6 opacity-80">
                {job.logs.map((log, j) => <li key={j} className="flex gap-4 items-start leading-relaxed"> <span className="text-sky-500 font-mono text-[10px] mt-1">0{j+1}</span> <span>{log}</span> </li>)}
              </ul>
            </motion.div>
          ))}
        </section>
